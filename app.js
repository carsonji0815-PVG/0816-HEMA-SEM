  ...
  function renderSystemStaffDirectory() {
    ...
    list.innerHTML = staffDirectory.map(staff => {
      ...
      return `<div class="system-staff-row">
        ...
        <label class="permission-switch system-staff-switch"><span><strong>${enabled ? (isAdmin ? "兼任当前项目会务负责人" : "已授权当前项目") : (isAdmin ? "未兼任当前项目会务负责人" : "未授权当前项目")}</strong><small>${staff.account_created ? (isAdmin ? "不影响超级管理员的全局最高权限" : "可随时开放或回收") : "请先在 Supabase 创建账号"}</small></span><span class="switch"><input type="checkbox" data-system-staff-email="${escapeHtml(staff.email)}" ${enabled ? "checked" : ""} ${staff.account_created ? "" : "disabled"}/><span></span></span></label>
      </div>`;
