...
for(const id of ["variflightDailyLimit","variflightUnlimited","variflightQuotaStatus"])assert.match(html,new RegExp(`id="${id}"`));
for(const token of ["variflightDailyLimit","variflightUnlimited","flightQuota","usedToday"])assert.ok(app.includes(token));
for(const token of ["flightUnlimited","flightDailyLimit","recordQuota","remaining"])assert.ok(moduleSource.includes(token));
