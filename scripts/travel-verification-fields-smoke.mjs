...
const attendee=F.applyLegacy({departDate:"2026-09-04",departCity:"上海",departTransportType:"PLANE",departStation:"上海虹桥机场T2航站楼",arriveDate:"2026-09-04",arriveCity:"北京",arriveTransportType:"PLANE",arriveStation:"北京首都机场T3航站楼",outNo:"MU5101",outDeparture:"09:00",outArrival:"11:00",returnDate:"2026-09-06",returnNo:"G101",returnFrom:"北京南站",returnTo:"上海虹桥站",returnDeparture:"12:00",returnArrival:"17:00"});
const plan=V.snapshot(attendee,"outbound");
assert.equal(V.buildCheck(attendee,"outbound",{found:true,match:plan}).status,"verified");
const different=V.buildCheck(attendee,"outbound",{found:true,match:{...plan,departure:"09:30",to:"北京大兴机场"}});
...
const six=V.buildCheck(attendee,"outbound",{found:true,match:{date:"2026-09-05",number:"MU5102",from:"银川河东机场T3航站楼",to:"北京大兴机场",departure:"13:00",arrival:"15:00"}});
