(function(root){
  ...
  function records(attendees,V){return attendees.flatMap(attendee=>segmentList(attendee,V).map(segment=>({attendee,segment,status:V.viewState(attendee,segment),data:V.snapshot(attendee,segment)})));}
  function render(attendees,V,{filter='all',query='',canEdit=false,isLocked=()=>false,selected=new Set()}={}){
    ...
    const selectableKeys=all.filter(item=>item.status!=='blank').map(item=>`${item.attendee.id}:${item.segment}`),visibleSelectableKeys=list.filter(item=>item.status!=='blank').map(item=>`${item.attendee.id}:${item.segment}`);
    const html=list.map(({attendee:a,segment,status,data})=>{
      ...
      const selectionKey=`${a.id}:${segment}`;
      const sameDirection=segmentList(a,V).filter(value=>segmentDirection(value,V)===segmentDirection(segment,V)),position=sameDirection.indexOf(segment)+1,directionLabel=segmentDirection(segment,V)==='return'?'返程':'去程',segmentLabel=sameDirection.length>1?`${directionLabel}第 ${position} 段`:directionLabel;
      return `<article class="verify-card ${selected.has(selectionKey)?'verify-card-selected':''}" data-verification-attendee="${escape(a.id)}" data-verification-segment="${escape(segment)}"><header><label class="verify-card-select"><input type="checkbox" data-select-verification="${escape(a.id)}" data-select-segment="${escape(segment)}" ${selected.has(selectionKey)?'checked':''} ${status==='blank'?'disabled':''}><span>核验此段</span></label><div><strong>${escape(a.name)} · ${escape(segmentLabel)}</strong><small>${escape(a.region||'')} · ${escape(data.number)}</small></div><span class="verify-state verify-state-${status}">${labels[status]}</span></header><div class="verify-table-scroll"><table class="verify-table"><thead><tr><th>名单字段</th><th>当前名单值</th><th>查询到的计划值</th><th>核验说明</th></tr></thead><tbody>${rows}</tbody></table></div><p class="verify-notice">${escape(status==='stale'?'名单已修改，旧核验结果不再适用。':(check?.notices||[]).join('；'))}</p><footer><small>${escape(source)}${at?' · '+escape(new Date(at).toLocaleString('zh-CN',{hour12:false})):''}</small><div><button class="button button-secondary" data-review-travel="${escape(a.id)}" data-review-segment="${escape(segment)}" ${editable?'':'disabled'}>${editable?'人工审核 / 修改':'只读 / 名单已锁定'}</button>${check&&editable?`<button class="text-button" data-reset-travel="${escape(a.id)}" data-reset-segment="${escape(segment)}">重置核验状态</button>`:''}</div></footer></article>`;
