(() => {
  ...
  function registrationQuotaRows(role=activeQuotaRole) {
    if(role==="角色嘉宾") {
      ...
      activeVisibleAttendees().filter(attendee=>normalizeQuotaRole(attendee.attendeeType)==="角色嘉宾").forEach(attendee=>{const item={venue:normalizeVenueLabel(attendee.venue)||"未填写会场",region:normalizeQuotaRegion(attendee.region),role:"角色嘉宾"};const key=quotaKey(item.venue,item.region,item.role);actualGroups.set(key,{...item,quota:0,actual:(actualGroups.get(key)?.actual||0)+1,gap:0,remaining:0,percent:0});});
