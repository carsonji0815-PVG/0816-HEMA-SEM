(() => {
  ...
  const initialState = () => ({
    ...
    settings: {
      ...
      fieldConfig: { title:true, hcpId:true, accommodation:true, flight:true, mslContact:true, remarks:true, clothingSize:false, serviceDeskName:"", serviceDeskStart:"09:00", serviceDeskEnd:"18:00", internalRoomingMode:"manual", registrationIdentityFields:[...DEFAULT_REGISTRANT_IDENTITY_FIELDS] },
  ...
  async function queryTransport(event) {
    ...
    result.innerHTML = renderLookupResult({name:maskName(a.name),accommodation:noStay?"无需住宿":"需要住宿",hotel:noStay?"无需住宿":a.customFields?.hotel||a.customFields?.酒店||"待安排",roomType:noStay?"无需住宿":room.assignedType?roomTypeLabel(room.assignedType):"待安排",checkInDate:noStay?"无需住宿":room.checkInDate||"待安排",checkOutDate:noStay?"无需住宿":room.checkOutDate||"待安排",outboundTransferOrigin:a.outboundTransferOrigin,outboundTransferTime:a.outboundTransferTime,outboundTransferNotes:a.outboundTransferNotes,outboundTransferDriverName:a.outboundTransferDriverName,outboundTransferDriverPhone:a.outboundTransferDriverPhone,outboundTransferVehicle:a.outboundTransferVehicle,returnTransferDestination:a.returnTransferDestination,returnTransferNotes:a.returnTransferNotes,returnTransferDriverName:a.returnTransferDriverName,returnTransferDriverPhone:a.returnTransferDriverPhone,returnTransferVehicle:a.returnTransferVehicle}, outbound, returnTrip, pickup, dropoff, state.settings);
  ...
  function renderLookupResult(attendeeInfo, outbound = {}, returnTrip = {}, pickup = {}, dropoff = {}, project = {}) {
    ...
    return `<div class="lookup-name">${escapeHtml(name)}，你的参会安排如下</div><section class="lookup-stay-summary" aria-label="住宿安排"><div class="lookup-stay-heading"><span>住宿安排</span><small>与分房管理最终安排同步</small></div><div class="lookup-stay-grid"><div><small>住宿酒店</small><strong>${escapeHtml(noStay?"无需住宿":info.hotel||"待安排")}</strong></div><div><small>房型</small><strong>${escapeHtml(noStay?"无需住宿":info.roomType||"待安排")}</strong></div><div><small>入住日期</small><strong>${escapeHtml(noStay?"无需住宿":info.checkInDate||"待安排")}</strong></div><div><small>退房日期</small><strong>${escapeHtml(noStay?"无需住宿":info.checkOutDate||"待安排")}</strong></div></div></section><section class="lookup-transport-group meeting"><div class="lookup-group-heading"><span>会议地接送安排</span><small>会议举办城市内执行</small></div><div class="lookup-transfer-grid">${card("接机 / 接站",outbound,pickup)}${card("送机 / 送站",returnTrip,dropoff)}</div></section>${localSection}<div class="calendar-action"><strong>还差一步：开启手机自动提醒</strong><span>受手机隐私规则限制，网页不能静默写入日历。请点击并在系统弹窗中确认“添加全部”。</span><button class="button button-primary button-block calendar-button" id="addCalendarButton" type="button">加入手机日历 · 接送前 30 分钟提醒</button></div>`;
  }
