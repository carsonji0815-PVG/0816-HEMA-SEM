(() => {
  ...
  function renderAttendeeTable() {
    ...
    const ticketStatusControl=a=>`<input class="progress-select ticket-status-input ${["ticketed","rail_ticketed","mixed_ticketed"].includes(a.ticketStatus)?"done":""}" type="text" list="ticketStatusSuggestions" value="${escapeHtml(ticketStatusLabel(a.ticketStatus))}" data-progress-field="ticketStatus" data-attendee-id="${a.id}" aria-label="出票状态，可选择或手动填写" placeholder="选择或输入出票状态" ${isLocked(a)||!canEditAttendeeData()||a.businessStatus==="cancelled"?"disabled":""}>`;
    ...
    const configuredTickets=normalizedTicketStatusOptions();
    ...
    $("#ticketStatusSuggestions")?.remove();const ticketStatusSuggestions=document.createElement("datalist");ticketStatusSuggestions.id="ticketStatusSuggestions";ticketStatusSuggestions.innerHTML=configuredTickets.map(item=>`<option value="${escapeHtml(item.label)}"></option>`).join("");$("#attendeeTableBody").after(ticketStatusSuggestions);
    if(ctaWorkflowEnabled())$$('[data-progress-field="ticketStatus"]',$("#attendeeTableBody")).forEach(select=>{const attendee=state.attendees.find(item=>item.id===select.dataset.attendeeId),cell=document.createElement("td");cell.innerHTML=progressSelect(attendee,"ctaStatus",[["pending","未完成"],["completed","已完成"]]);select.closest("td")?.before(cell);});
