...
test('saved meeting feature survives reload; normalization is safe during bootstrap',async()=>{
 ...
 w.eval(await readFile(new URL('luggage-integration.js',root),'utf8'));w.eval(await readFile(new URL('app.js',root),'utf8'));w.document.dispatchEvent(new w.Event('DOMContentLoaded'));await pause()
 assert.equal(w.document.querySelectorAll('#luggageFrame').length,1)
