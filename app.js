(() => {
  ...
  function renderAttendeeTable() {
    ...
    $$('[data-progress-field="ticketStatus"]',$("#attendeeTableBody")).forEach(select=>{const attendee=state.attendees.find(item=>item.id===select.dataset.attendeeId);select.innerHTML=configuredTickets.map(item=>`<option value="${escapeHtml(item.id)}" ${attendee?.ticketStatus===item.id?"selected":""}>${escapeHtml(item.label)}</option>`).join("");if(["ticketed","rail_ticketed","mixed_ticketed"].includes(attendee?.ticketStatus))select.classList.add("done");});
