...
- 项目设置内维护会议总名额及“会场＋大区＋参会角色”分组名额；大区可提前预设，也可在分配时自由输入并自动沉淀，报名期间可持续调整，进度看板即时重算缺口、超额和完成率
- 报名进度页签分为“听众 / 角色嘉宾”；角色嘉宾页签内的会议角色按名单实际值分别统计主席、主持、讲者、讨论嘉宾（组长），未细分的旧数据单列为“角色嘉宾（未细分）”；角色嘉宾不设名额限制，仅按有效名单汇总
- 当前参会名单默认排除已删除/已取消报名；独立归档入口按原报名模板完整展示全部字段
...
- 手动修正：仅本次检出的异常字段标色，显示计划值；保存人工修改后返回行程核验板块；变更方向待重查，不自动调用收费接口。名单对应行程单元格显示持久“已核验”标记，字段修改后旧标记自动失效。
