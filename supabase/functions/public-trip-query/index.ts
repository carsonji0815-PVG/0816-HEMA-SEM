...
const normalizeDate=(value:unknown)=>{
  const match=clean(value,20).replace(/[\u5e74/.]/gu,"-").replace(/\u6708/gu,"-").replace(/\u65e5/gu,"").match(/^(\d{1,4})-(\d{1,2})-(\d{1,2})$/);if(!match)return"";
  let year=Number(match[1]);const month=Number(match[2]),day=Number(match[3]);if(year>=0&&year<100)year+=2000;
  if(year<2000||year>2099||month<1||month>12||day<1||day>31)return"";
  const date=new Date(Date.UTC(year,month-1,day));if(date.getUTCFullYear()!==year||date.getUTCMonth()!==month-1||date.getUTCDate()!==day)return"";
...
const normalizeRoomType = (value: unknown) => ({single:"single",shared:"shared",twin_single:"twin_single",none:"none",单间:"single",标间拼住:"shared",标间单住:"twin_single",无需住宿:"none"}[clean(value,30)] || "");
...
fetch: withSupabase({ auth: ["publishable", "secret"] }, async request => {
  ...
  if (action === "save-attendee") {
    ...
    const requestedRoomType=normalizeRoomType(details.roomType)||(!yes(details.accommodation)?"none":"");
    const customFields = {...internalCustom,...Object.fromEntries(Object.entries(requestedCustom).filter(([key])=>allowedCustom.has(key)).slice(0,50).map(([key,value])=>[key,clean(value,500)])),...(requestedRoomType?{roomType:requestedRoomType}:{}),_journeySegments:journeySegments,_location:locationReference(meeting,details.venue,(internalCustom._location||{}) as Record<string,unknown>)};
    ...
    const values: Record<string,unknown> = {
      attendee_type:clean(details.attendeeType,30) || "HCP", name:attendeeName, city:clean(details.city,50), hospital:clean(details.hospital,100), department:clean(details.department,100), title:clean(details.title,50), venue:clean(details.venue,50), sex:clean(details.sex,10), id_number:clean(details.idNumber,100), phone:attendeePhone, hcp_id:clean(details.hcpId,100), accommodation:requestedRoomType!=="none"&&yes(details.accommodation), is_flight:yes(details.flight), region:clean(registrant.region,50),
      depart_date:normalizeDate(details.departDate)||null,depart_city:clean(details.departCity,50),depart_transport_type:departType||null,depart_station:departStation,
      arrive_date:normalizeDate(details.arriveDate)||null,arrive_city:clean(details.arriveCity,50),arrive_transport_type:arriveType||null,arrive_station:arriveStation,
      return_depart_date:normalizeDate(details.returnDepartDate)||null,return_depart_city:clean(details.returnDepartCity,50),return_depart_transport_type:returnDepartType||null,return_depart_station:returnDepartStation,
      return_arrive_date:normalizeDate(details.returnArriveDate)||null,return_arrive_city:clean(details.returnArriveCity,50),return_arrive_transport_type:returnArriveType||null,return_arrive_station:returnArriveStation,
      out_date:normalizeDate(details.departDate) || null, out_from:departStation||clean(details.departCity,50), out_to:arriveStation||clean(details.arriveCity,50), out_no:clean(details.outNo,30), out_departure:clean(details.outDeparture,5) || null, out_arrival:clean(details.outArrival,5) || null,
      return_date:normalizeDate(details.returnDepartDate) || null, return_from:returnDepartStation||clean(details.returnDepartCity,50), return_to:returnArriveStation||clean(details.returnArriveCity,50), return_no:clean(details.returnNo,30), return_departure:clean(details.returnDeparture,5) || null, return_arrival:clean(details.returnArrival,5) || null,
    ...
    const missingRequired=templateColumns.filter(column=>column.required).some(column=>{const key=clean(column.key,80);const value=key==="roomType"?requestedRoomType:column.custom ? customFields[key] : values[keyToDb[key]];return value === null || value === undefined || value === "";});
    const validDate=(value:unknown)=>normalizeDate(value)===value;
    ...
    if (missingRequired||journeyMissing||(yes(details.accommodation)&&!requestedRoomType)) return reply({ error:"请完整填写当前项目报名模板、申请房型和行程信息" }, 400);
    ...
    if (attendee) {
      ...
      const lockedChange=(locks || []).find(lock=>(groups[lock.field_group] || []).some(key=>String(attendee?.[key] ?? "") !== String(values[key] ?? ""))||lock.field_group==="accommodation"&&normalizeRoomType((attendee.custom_fields as Record<string,unknown>)?.roomType)!==requestedRoomType||["outbound","return"].includes(lock.field_group)&&JSON.stringify(oldSegments.filter(item=>(item.direction==="return"?"return":"outbound")===lock.field_group))!==JSON.stringify(journeySegments.filter(item=>item.direction===lock.field_group)));
  ...
  if (clean(payload.action) === "complete-registration") {
    ...
    const allowedCustom = new Set(((meeting.registration_template as {columns?:Array<{key?:string,custom?:boolean}>})?.columns || []).filter(column=>column.custom).map(column=>clean(column.key,80)));
    const requestedRoomType=normalizeRoomType(payload.roomType)||(!yes(payload.accommodation)?"none":"");
    ...
    const values = {
      attendee_type:clean(payload.attendeeType,30), city:clean(payload.city,50), hospital:clean(payload.hospital,100), department:clean(payload.department,100), title:clean(payload.title,50), venue:clean(payload.venue,50), sex:clean(payload.sex,10), id_number:clean(payload.idNumber,100), hcp_id:clean(payload.hcpId,100), accommodation:requestedRoomType!=="none"&&yes(payload.accommodation), is_flight:yes(payload.flight),
      out_date:normalizeDate(payload.outDate) || null, out_from:clean(payload.outFrom,50), out_to:clean(payload.outTo,50), out_no:clean(payload.outNo,30), out_departure:clean(payload.outDeparture,5) || null, out_arrival:clean(payload.outArrival,5) || null,
      return_date:normalizeDate(payload.returnDate) || null, return_from:clean(payload.returnFrom,50), return_to:clean(payload.returnTo,50), return_no:clean(payload.returnNo,30), return_departure:clean(payload.returnDeparture,5) || null, return_arrival:clean(payload.returnArrival,5) || null,
    ...
    const missingRequired=templateColumns.filter(column=>column.required).some(column=>{const key=clean(column.key,80);return key==="roomType"?!requestedRoomType:column.custom?!customFields[key]:keyToDb[key]&&!values[keyToDb[key] as keyof typeof values];});
    const contactRequired=templateColumns.some(column=>column.key==="contactMobile"&&column.required);
    if (missingRequired || yes(payload.accommodation)&&!requestedRoomType || (contactRequired && !/^1\d{10}$/.test(values.contact_mobile))) return reply({ error:"请完整填写报名表中的必填信息和申请房型" }, 400);
  ...
  const requestedRoomType=normalizeRoomType(attendeeCustomFields.roomType||finalRooming.requestedType||"");
  ...
  return reply({
    ...
    attendee: { name: `${attendee.name.slice(0, 1)}${"*".repeat(Math.max(attendee.name.length - 1, 1))}`, accommodation:noStay?"无需住宿":"需要住宿", hotel:finalHotel, meetingVenue:assignedVenue?.name||"待公布", requestedRoomType:noStay?"无需住宿":roomTypeLabels[requestedRoomType]||"待补录", roomType:finalRoomType, checkInDate:finalCheckIn, checkOutDate:finalCheckOut, outboundTransferOrigin:attendee.outbound_transfer_origin||"", outboundTransferTime:String(attendee.outbound_transfer_time||"").slice(0,16), outboundTransferNotes:attendee.outbound_transfer_notes||"", outboundTransferDriverName:attendee.outbound_transfer_driver_name||"", outboundTransferDriverPhone:attendee.outbound_transfer_driver_phone||"", outboundTransferVehicle:attendee.outbound_transfer_vehicle||"", returnTransferDestination:attendee.return_transfer_destination||"", returnTransferNotes:attendee.return_transfer_notes||"", returnTransferDriverName:attendee.return_transfer_driver_name||"", returnTransferDriverPhone:attendee.return_transfer_driver_phone||"", returnTransferVehicle:attendee.return_transfer_vehicle||"" },
