...
await page.route("**/functions/v1/public-trip-query",route=>route.fulfill({status:200,contentType:"application/json",body:JSON.stringify({
  ...
  outbound:{number:"CZ5828",from:"上海",fromStation:"上海浦东机场T2航站楼",to:"长沙",toStation:"长沙黄花机场T2航站楼",date:"2026-11-27"},
  returnTrip:{number:"MU5364",from:"长沙",fromStation:"长沙黄花机场T2航站楼",to:"上海",toStation:"上海虹桥机场T2航站楼",date:"2026-11-28"},
  transports:[
...
console.log(JSON.stringify({layout:"pass",headerAboveQuery:true,meetingRow:Math.round(meetingBox.width),localRow:Math.round(localBox.width),placardThumbnail:"pass",overflow,errors},null,2));
await browser.close();
