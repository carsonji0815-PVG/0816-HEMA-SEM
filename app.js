(() => {
  ...
  function renderSystemStaffDirectory() {
    ...
    list.innerHTML = staffDirectory.map(staff => {
      ...
      return `<div class="system-staff-row">
        ...
        <div class="system-staff-badges"><span class="status ${staff.account_created ? "status-normal" : "status-locked"}">${accountState}</span>${isAdmin ? `<span class="status status-ok">全部项目 · 最高权限</span>` : `<label>系统角色<select data-system-staff-role="${escapeHtml(staff.email)}"><option value="ops" ${isReadonly?"":"selected"}>会务负责人</option><option value="readonly" ${isReadonly?"selected":""}>只读查看</option></select></label>`}</div>
  ...
  async function syncBackend() {
    ...
    if (canManage()) {
      const { error } = await backend.from("meetings").update({ name:state.settings.eventName, activity_type:state.settings.activityType||"external", project_identifier:state.settings.identifier||state.settings.slug, activity_owner:state.settings.activityOwner||null, activity_date:state.settings.activityDate||state.settings.startDate||null, end_date:state.settings.endDate||null, start_date:state.settings.startDate||null, client_name:state.settings.clientName||null, venues:state.settings.venues, service_phone:state.settings.servicePhone||null, brand_color:state.settings.brandColor, deadline:state.settings.deadline||null, capacity:state.settings.capacity, allowed_departure_cities:state.settings.allowedCities, check_city_mismatch:state.settings.mismatchRule, check_departure_city:false, flight_lead_minutes:state.settings.flightLeadMinutes, train_lead_minutes:state.settings.trainLeadMinutes, transport_group_minutes:state.settings.transportGroupMinutes||30, transfer_collection_enabled:!!state.settings.transferCollectionEnabled, transfer_collection_roles:state.settings.transferCollectionRoles||[], field_config:{...state.settings.fieldConfig,registrationQuotas:state.settings.registrationQuotas||[],quotaRegions:state.settings.quotaRegions||[],travelApprovalRules:state.settings.travelApprovalRules||{},roomingRules:state.settings.roomingRules||{},transportStationRules:state.settings.transportStationRules||[]}, master_locked:state.locks.master }).eq("id",backendMeetingId); if (error) throw error;
  ...
  function exportRoomingOccupancy(){if(isReadOnlyStaff())return toast("只读账号没有数据导出权限","error");if(!window.XLSX)return toast("Excel 组件尚未加载，请刷新后重试","error");const data=roomingOccupancyData();if(!data.rows.length)return toast("当前日期范围内没有可导出的分房数据","error");const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,roomingOccupancySheet(data),"分房统计");XLSX.writeFile(wb,`${state.settings.slug||"项目"}-分房统计-${data.from}-${data.to}.xlsx`);toast("分房统计已导出");}
