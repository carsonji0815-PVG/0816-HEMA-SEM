...
server.listen(PORT, HOST, () => { console.log('礼来会务文件系统已启动（SQLite）：'); console.log(`  本机访问：http://127.0.0.1:${PORT}`); const addresses = Object.values(os.networkInterfaces()).flat().filter((item) => item && item.family === 'IPv4' && !item.internal); for (const address of addresses) console.log(`  局域网访问：http://${address.address}:${PORT}`); });
