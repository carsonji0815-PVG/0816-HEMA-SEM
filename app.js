(() => {
  ...
  function renderRoomingOccupancy(){
    ...
    $("#roomingOccupancyBody").innerHTML=data.rows.length?data.rows.map(row=>`<tr><td>${escapeHtml(roomingOccupancyDateLabel(row.date))}</td><td>${row.single}</td><td>${row.twinSingle}</td><td>${row.shared}</td><td class="rooming-day-total">${row.single+row.twinSingle+row.shared}</td></tr>`).join(""):`<tr><td class="rooming-occupancy-empty" colspan="5">当前日期范围内暂无已完成的住宿安排</td></tr>`;
  }
  function exportRoomingOccupancy(){if(!window.XLSX)return toast("Excel 组件尚未加载，请刷新后重试","error");const data=roomingOccupancyData();if(!data.rows.length)return toast("当前日期范围内没有可导出的分房数据","error");const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,roomingOccupancySheet(data),"分房统计");XLSX.writeFile(wb,`${state.settings.slug||"项目"}-分房统计-${data.from}-${data.to}.xlsx`);toast("分房统计已导出");}
