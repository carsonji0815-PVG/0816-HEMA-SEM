(() => {
  ...
  function roomingDates(attendee){return RoomingEngine.stayDates(attendee);}
  ...
  function updateRoomingField(id,field,value) {
    ...
    const fieldLabel={assignedType:"实际房型",roommateId:"拼住室友",actualNights:"实际允许住宿间夜",roomNumber:"房号"}[field]||field;
  ...
  function renderRooming() {
    ...
    $("#roomingMetrics").innerHTML=[["住宿人员",staying.length,"来源于有效参会名单","⌂"],["实际总间夜",staying.reduce((sum,a)=>sum+(Number(roomingRecord(a).actualNights)||0),0),"统一用于导出与财务核算","☾"],["待人工安排",pendingManual.length,"自动拼住无匹配或资料不全","!"],["住宿待审批",pendingApproval.length,"需在审批中心处理","⚑"]].map(([label,value,note,icon])=>`<article class="metric-card"><span class="metric-icon">${icon}</span><small>${label}</small><strong>${value}</strong><p>${note}</p></article>`).join("");
  ...
  function exportRoomingList(){const headers=["序号","姓名","性别","大区","省份","城市","医院","职称","会场","入住日期","撤离日期","参考间夜","实际允许住宿间夜数","申请房型","系统建议","实际房型","拼住室友","匹配依据","房号","安排状态","住宿审批"];const rows=activeVisibleAttendees().filter(a=>a.accommodation==="Y"||roomingRecord(a).requestedType||roomingRecord(a).assignedType).map((a,index)=>{const room=roomingRecord(a),dates=roomingDates(a);return[index+1,a.name,a.sex,a.region,RoomingEngine.province(a),a.city,a.hospital,a.title,a.venue,dates.checkIn,dates.checkOut,roomingNights(a),room.actualNights,roomTypeLabel(room.requestedType),roomTypeLabel(suggestedRoomType(a)),roomTypeLabel(room.assignedType),state.attendees.find(item=>item.id===room.roommateId)?.name||"",room.pairingReason||"",room.roomNumber,room.assignedType==="shared"&&!room.roommateId?"待人工安排":"已安排",roomingApprovalStatus(a)==="approved"?"已批准":roomingApprovalStatus(a)==="pending"?"待审批":roomingApprovalStatus(a)==="rejected"?"已退回":"无需审批"]});if(window.XLSX){const ws=XLSX.utils.aoa_to_sheet([headers,...rows]);ws["!cols"]=headers.map((header,index)=>({wch:[6,7,16,17].includes(index)?24:14}));const wb=XLSX.utils.book_new();XLSX.utils.book_append_sheet(wb,ws,"Rooming List");XLSX.writeFile(wb,`${state.settings.slug||"项目"}-Rooming-List.xlsx`);toast("Rooming List 已导出，实际间夜已用于统计");}}
  ...
  function saveSettings(event) {
    ...
    addNotification("change", `${currentUser().name}更新了当前项目的行程审批与分房规则`); saveState(); populateProjects(); renderAll(); toast("项目设置已保存；如需应用新分房规则，请在分房管理重新执行自动分房");
  }
